...
export default function ShareFormModal({ 
  ...
  const loadProjects = async () => {
    try {
      ...
      // Buscar o projeto atual do formulário para obter o companyId
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${(import.meta as any).env?.VITE_API_URL || 'https://labore-api.onrender.com/api'}/forms/${form.id}/share`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Erro ao compartilhar formulário');
      }
      ...
      alert(error.message || 'Erro ao compartilhar formulário');
    } finally {
      setSaving(false);
    }
  };
